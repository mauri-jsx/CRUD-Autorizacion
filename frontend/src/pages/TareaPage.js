...
export function renderTareaPage() {
    ...
    try {
      ...
      tareas.forEach((tarea) => {
        ...
        li.innerHTML = `
          ...
          <button data-id="${
            tarea._id
          }" class="edit bg-yellow-500 text-white px-3 py-1 rounded mr-2">Editar</button>
          <button data-id="${
            tarea._id
          }" class="delete bg-red-500 text-white px-3 py-1 rounded">Eliminar</button>
        `;
          ...
          try {
            ...
            const { value: formValues } = await Swal.fire({
              ...
              html:
                ...
                `<textarea id="swal-input2" class="swal2-textarea" placeholder="Descripción">${
                  tarea.descripcion || ""
                }</textarea>`,
              focusConfirm: false,
  ...
  tareaForm.addEventListener("submit", async (e) => {
    ...
    try {
      await createTarea({ nombre, descripcion, fecha });
      tareaForm.reset();
